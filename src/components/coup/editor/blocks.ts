...
const BLOCKS: Record<string, unknown>[] = [
  ...
  // Styled like function definitions: a title line with the real python name
  // (blocks↔code is a learning goal) + a muted "inputs" line of parameter chips.
  {
    type: 'coup_when_turn',
    message0: "⚔ your_turn — when it's my turn",
    message1: 'inputs: %1',
    args1: [{ type: 'field_label', name: 'IN', text: 'state (the whole game)', class: 'ed-hat-inputs' }],
    message2: 'do %1',
    args2: [{ type: 'input_statement', name: 'DO' }],
    colour: C_EVENT,
    tooltip: 'Runs on your turn — return exactly one action. Read the game with state.* blocks from the Game Info category.',
    hat: 'cap',
  },
  {
    type: 'coup_when_respond',
    message0: '🛡 respond — my opponent makes a move',
    message1: 'inputs: %1',
    args1: [{ type: 'field_label', name: 'IN', text: 'the move: who made it · what it is · role they claim · is it a block · who is blocking', class: 'ed-hat-inputs' }],
    message2: 'do %1',
    args2: [{ type: 'input_statement', name: 'DO' }],
    colour: C_EVENT,
    tooltip: 'Your opponent moved — challenge / block(role) / allow. Grab every "the move" input from the This Move category; game facts live in Game Info.',
    hat: 'cap',
  },
  {
    type: 'coup_when_assassinated',
    message0: '🗡 when_assassinated — they call one of my cards',
    message1: 'inputs: %1',
    args1: [{ type: 'field_label', name: 'IN', text: 'the move: the role they called', class: 'ed-hat-inputs' }],
    message2: 'do %1',
    args2: [{ type: 'input_statement', name: 'DO' }],
    colour: C_EVENT,
    tooltip: 'They named a card. If it is NOT one you hold it misses anyway — allow() (dodge) is often right. Get "the role they called" from the This Move category.',
    hat: 'cap',
  },
  {
    type: 'coup_choose_lose',
    message0: '💀 choose_card_to_lose — I must give up a card',
    message1: 'inputs: %1',
    args1: [{ type: 'field_label', name: 'IN', text: 'my cards', class: 'ed-hat-inputs' }],
    message2: 'do %1',
    args2: [{ type: 'input_statement', name: 'DO' }],
    colour: C_EVENT,
    tooltip: 'Pick which card to flip into your graveyard. "my cards" and the card pickers are in Game Info.',
    hat: 'cap',
  },
  {
    type: 'coup_choose_exchange',
    message0: '⚜ choose_exchange — pick my new hand',
    message1: 'inputs: %1',
    args1: [{ type: 'field_label', name: 'IN', text: 'the pool (my cards + drawn)', class: 'ed-hat-inputs' }],
    message2: 'do %1',
    args2: [{ type: 'input_statement', name: 'DO' }],
    colour: C_EVENT,
    tooltip: 'Ambassador exchange: return the list of role names to keep (exactly your card count). "the exchange pool" is in the Responses / Game Info blocks.',
    hat: 'cap',
  },
  ...
  const toolbox = {
    ...
    contents: [
      ...
      },
      {
        ...
        contents: [
          { kind: 'label', text: 'These are the inputs of respond / when_assassinated' },
