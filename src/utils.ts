const studentDeadline = new Date('2025-03-10');
...
const formatDateToReadableString = (date: Date): string => {
  const localDate = new Date(date.toLocaleString('default', { timeZone: 'UTC' })); // Convert to local time
  const monthNames = [
...
const faqs: FAQItemObj[] = [
  {
    ...
    answer: `Week 1 will run from ${formatDateToReadableString(new Date(week1Start.toLocaleString()))} to ${formatDateToReadableString(new Date(week1End.toLocaleString()))}, hosting rising 6th/7th graders. Week 2 will run from ${formatDateToReadableString(new Date(week2Start.toLocaleString()))} to ${formatDateToReadableString(new Date(week2End.toLocaleString()))}, hosting rising 8th/9th graders. Our program is fully in-person on MIT campus!`  
  },
  {
